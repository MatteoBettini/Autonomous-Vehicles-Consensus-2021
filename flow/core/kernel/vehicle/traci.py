...
class TraCIVehicle(KernelVehicle):
    ...
    def get_accel(self, veh_id, noise=True, failsafe=True):
        """See parent class."""
        if isinstance(veh_id, (list, np.ndarray)):
            return [self.get_accel(vehID, noise, failsafe) for vehID in veh_id]
        metric_name = 'accel'
        if noise:
    ...
    def get_realized_accel(self, veh_id):
        """See parent class."""
        if isinstance(veh_id, (list, np.ndarray)):
            return [self.get_realized_accel(vehID) for vehID in veh_id]
    ...
    def get_distance(self, veh_id, error=-1001):
        """See parent class."""
        if isinstance(veh_id, (list, np.ndarray)):
            return [self.get_distance(vehID) for vehID in veh_id]
